...
##
CALL gds.pageRank.stream('Missions') YIELD nodeId, score AS pageRank
WITH gds.util.asNode(nodeId) AS n, pageRank
WHERE n.mission_name IS NOT NULL
MATCH (n)-[i:ACCOMPLISHED_BY_SUBSYSTEM]-()
RETURN n.mission_name AS name, pageRank, count(i) AS interactions
